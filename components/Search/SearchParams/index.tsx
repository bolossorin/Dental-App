...
export const SearchParams: React.FC<ISearchParamsProps> = ({
  ...
  const handleSearch = async () => {
    await findDentist(service, miles);
  };
  ...
  useEffect(
    () => {
      if (!!debouncedSearchTerm) {
        fetch(
          `https://maps.google.com/maps/api/geocode/json?sensor=false&address=${debouncedSearchTerm}&key=${process.env.GOOGLE_KEY}`
        )
          .then((res) => res.json())
          .then((res) => {
        ...
        const oldLocation = JSON.parse(
          localStorage.getItem("myLocation") || "false"
        );
        ...
        <div className="box-left">
          ...
          <p className="row-content-index">
            <select
              ...
              id="services"
              onChange={(e) => {
                SetMiles(e.target.value as any);
              }}
            >
              <option value={""} selected>
                Select Mile
          ...
          <button className="button-green-search" onClick={handleSearch}>
            Find My Dentist
          </button>
        </div>
