...
export const Watermark: React.FC<UploadWatermarkProps> = ({setWatermarkFile, onFire, disableUpload}) => {
  ...
  const handleAfterImgChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      const _URL = window.URL || window.webkitURL;
      const file = e.target.files && e.target.files[0];
      const binaryData: any = [];
      binaryData.push(file);
      const objectUrl = _URL.createObjectURL(new Blob(binaryData, {type: "application/zip"}));
  ...
  return (
    <div className="profile-box-form cut-block">
      ...
      <div className="profile-block-box">
        {watermarkSrc && (<div className="form-login-buttons">
          ...
          <div className="d-flex">
            ...
            <button type='button' className="gallery-button-green" onClick={() => setWatermarkSrc("")}>
              Cancel
            </button>
          </div>
        </div>)}
          ...
          <div className="gallery-block-image">
            <p className="gallery-upload">
              ...
              <input
                ...
                name="cover_image"
                id="cover_image"
                onChange={(e) => handleAfterImgChange(e)}
